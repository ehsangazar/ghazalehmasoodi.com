...
if ( ! function_exists( 'influencer_setup' ) ) :
	...
	function influencer_setup() {
		...
		add_image_size( 'influencer-schema', 600, 60, true );
...
function influencer_scripts() {
	...
	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/css' . $unminify . '/font-awesome' . $suffix . '.css', array(), '4.7'  );
    ...
    wp_enqueue_script( 'fontawesome-all', get_template_directory_uri() . '/js' . $unminify . '/fontawesome-all' . $suffix . '.js', array( 'jquery' ), '5.0.13', true );
    wp_enqueue_script( 'fontawesome-shim', get_template_directory_uri() . '/js' . $unminify . '/fontawesome-shim' . $suffix . '.js', array( 'jquery' ), '5.2.0', true );
...
function influencer_change_comment_form_default_fields( $fields ){
    ...
    $fields['author'] = '<p class="comment-form-author"><input id="author" name="author" placeholder="' . esc_attr__( 'Name*', 'influencer' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['email'] = '<p class="comment-form-email"><input id="email" name="email" placeholder="' . esc_attr__( 'Email*', 'influencer' ) . '" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['url'] = '<p class="comment-form-url"><input id="url" type="url" name="url" placeholder="' . esc_attr__( 'Website', 'influencer' ) . '" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>'; 
...
function influencer_change_comment_form_defaults( $defaults ){
    ...
    $defaults['comment_field'] = '<p class="comment-form-comment"><textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Comment', 'influencer' ) . '" cols="40" rows="8" required="required"></textarea></p>';
...
if( ! function_exists( 'influencer_single_post_schema' ) ) :
/**
 * Single Post Schema
 *
 * @return string
 */
function influencer_single_post_schema() {
    if ( is_singular( 'post' ) ) {
        global $post;
        $custom_logo_id = get_theme_mod( 'custom_logo' );

        $site_logo   = wp_get_attachment_image_src( $custom_logo_id , 'influencer-schema' );
        $images      = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
        $excerpt     = influencer_escape_text_tags( $post->post_excerpt );
        $content     = $excerpt === "" ? mb_substr( influencer_escape_text_tags( $post->post_content ), 0, 110 ) : $excerpt;
        ...
        $args = array(
            ...
            "author"        => array(
                "@type"     => "Person",
                "name"      => influencer_escape_text_tags( get_the_author_meta( 'display_name', $post->post_author ) )
            ),
            "publisher" => array(
                "@type"       => "Organization",
...
add_action( 'wp_head', 'influencer_single_post_schema' );
