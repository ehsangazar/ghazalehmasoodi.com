...
function influencer_save_sidebar_layout( $post_id ){
    global $influencer_sidebar_layout , $post;
    ...
    foreach( $influencer_sidebar_layout as $field ){  
        ...
        $old = get_post_meta( $post_id, '_influencer_sidebar_layout', true ); 
        $new = sanitize_text_field( $_POST['influencer_sidebar_layout'] );
        if( $new && $new != $old ) {  
            update_post_meta( $post_id, '_influencer_sidebar_layout', $new );  
        }elseif( '' == $new && $old ) {  
            delete_post_meta( $post_id, '_influencer_sidebar_layout', $old );  
        } 
     } // end foreach     
}
